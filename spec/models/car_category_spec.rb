...
RSpec.describe CarCategory, type: :model do
  context 'validation' do
    it 'attribute cannot be blank' do
      ...
      expect(car_category.errors[:third_party_insurance]).to include('Todos atributos não podem ficar em branco')
    end

    it 'name must be uniq' do
      CarCategory.create!(name: 'Hatch', daily_rate: 5.1,car_insurance: 300, third_party_insurance: 100)
      car_category = CarCategory.new(name: 'Hatch', daily_rate: 5.1,car_insurance: 300, third_party_insurance: 100)
